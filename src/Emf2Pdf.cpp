...
const char *emfNames[] = {"EMR_HEADER",                  /*1*/
                          "EMR_POLYBEZIER",              /*2*/
                          "EMR_POLYGON",                 /*3*/
                          "EMR_POLYLINE",                /*4*/
                          "EMR_POLYBEZIERTO",            /*5*/
    ...
    else
    {
        ...
        if (obj < nCreated)
        {
            ...
            case OBJ_PEN:
                ...
                HPDF_Page_SetRGBStroke(page, currentPen.r, currentPen.g, currentPen.b);
                HPDF_Page_SetDash(page, (const HPDF_REAL *)currentPen.dash, currentPen.nDash, 0);
                HPDF_Page_SetLineCap(page, (HPDF_LineCap)currentPen.end);
                HPDF_Page_SetLineJoin(page, (HPDF_LineJoin)currentPen.join);
                break;
            case OBJ_BRUSH:
...
size_t Emf2Pdf::CreatePen()
{
    ...
    case PS_ENDCAP_SQUARE:
        created[idx].pen.end = HPDF_PROJECTING_SQUARE_END;
        break;
    case PS_ENDCAP_FLAT:
...
int Emf2Pdf::FindFont(wchar_t *faceName, bool bold, bool italic)
{
    ...
    {
        if (wcswcs(installedFont[i].name, faceName) == 0)
            continue;
...
const char *Emf2Pdf::GetEncoding(unsigned char idx)
{
    ...
    case EASTEUROPE_CHARSET:
        return "CP1250"; // Microsoft Windows Codepage 1250 (EE)
    case RUSSIAN_CHARSET:
        return "CP1251"; // Microsoft Windows Codepage 1251 (Cyrl)
    // case RUSSIAN_CHARSET:		return ("KOI8-R"); break;
    case ANSI_CHARSET:
        return "CP1252"; // Microsoft Windows Codepage 1252 (ANSI)
    case GREEK_CHARSET:
        return "CP1253"; // Microsoft Windows Codepage 1253 (Greek)
    case TURKISH_CHARSET:
        return "CP1254"; // Microsoft Windows Codepage 1254 (Turk)
    case HEBREW_CHARSET:
        return "CP1255"; // Microsoft Windows Codepage 1255 (Hebr)
    case ARABIC_CHARSET:
        return "CP1256"; // Microsoft Windows Codepage 1256 (Arab)
    case BALTIC_CHARSET:
        return "CP1257"; // Microsoft Windows Codepage 1257 (BaltRim)
    case VIETNAMESE_CHARSET:
        return "CP1258"; // Microsoft Windows Codepage 1258 (Viet)
    }
    return 0;
}
...
size_t Emf2Pdf::CreatePDFFont()
{
    ...
    fread(&cItalic, 1, 1, f);
    ret += 1;
    fread(&underline, 1, 1, f);
    ret += 1;
    fread(&strikeOut, 1, 1, f);
    ret += 1;
    ...
    if (i >= 0)
    {
        ...
        if (created[idx].font.height > 100)
            created[idx].font.height = 100;
        ...
        if (width != 0)
        {
            ...
            HPDF_Font_MeasureText(created[idx].font.font,
                                  (const HPDF_BYTE *)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789",
                                  62, 1e10, created[idx].font.height, 0, 0, HPDF_FALSE, &currWidth);
            currWidth /= 62;
...
size_t Emf2Pdf::PDFTextOut(unsigned long code)
{
    ...
    fread(&font_xScale, 4, 1, f);
    ret += 4;
    fread(&font_yScale, 4, 1, f);
    ret += 4;
    POINTL pos;
    fread(&pos, 8, 1, f);
    ret += 8;
    ...
    dxOff -= 8;
    if (strOff > ret)
        fseek(f, (long)(strOff - ret), SEEK_CUR);
    ret = strOff;
    ...
    if (code == EMR_EXTTEXTOUTW)
    {
        ...
        strw[nChar] = 0;
        fread(strw, sizeof(wchar_t), nChar, f);
        ret += sizeof(wchar_t) * nChar;
        ...
        if (ll <= 1 && utf16len > 0)
        {
            ...
            if (currentFont->encoding != 1)
            {
                ...
                const char *loaded =
                    HPDF_LoadTTFontFromFile(pdf, installedFont[currentFont->intalledIdx].path, this->embedFont);
    ...
    if (dxOff > ret)
        fseek(f, (long)(dxOff - ret), SEEK_CUR);
    ret = dxOff;
    PRINT_DBG("  %i,%i -> %s\r\n", pos.x, pos.y, str);
    if ((signed)ll <= 0)
        return ret;
    unsigned long *dx = (unsigned long *)malloc(sizeof(unsigned long) * nChar);
    fread(dx, sizeof(unsigned long), nChar, f);
    ret += sizeof(unsigned long) * nChar;
    char out[5];
    memset(out, 0, 5);
    ...
    for (int step = 0; step < 2; step++)
    {
        ...
        else if ((textAlign & TA_CENTER) == TA_CENTER && (step == 1))
        {
            ...
            for (i = 0; i < nChar && i < ll; i++)
            {
                ...
                if (j >= 0b11110000)
                    n = 4;
                else if (j >= 0b11100000)
                    n = 3;
                else if (j >= 0b11000000)
                    n = 2;
                else
                    n = 1;
                for (j = 0; j < n; j++, i++)
...
size_t Emf2Pdf::StretchDiBits(unsigned long code)
{
    ...
    {
        fread(&bmi, sizeof(BITMAPINFOHEADER), 1, f);
...
void Emf2Pdf::ParseFile()
{
    while (!feof(f))
    {
        ...
        PRINT_DBG("Record %i(%s) length %i (gMode 0x%04X)\r\n", code, emfNames[code - 1], size,
                  page ? HPDF_Page_GetGMode(page) : 0);
        switch (code)
        {
        case EMR_HEADER:
            nRead += Header();
        ...
        case EMR_STROKEANDFILLPATH:
            nRead += StrokeAndFillPath();
            break;
        case EMR_SELECTCLIPPATH:
            nRead += SelectClipPath();
            break;
        case EMR_SETPOLYFILLMODE:
            nRead += SetPolyFillMode();
            break;
...
void Emf2Pdf::InitInstalledFont()
{
    if (installedFont != 0)
        return;
    HKEY hFonts;
    LSTATUS result = RegOpenKeyExA(HKEY_LOCAL_MACHINE, "Software\\Microsoft\\Windows NT\\CurrentVersion\\Fonts", 0,
                                   KEY_READ, &hFonts);
